...
export default function ShowMatchPanel({ initialPhotoMetadata, initialShowResult, initialSharedPhoto = null }) {
  ...
  const flowSteps = [
    ...
    { id: 'date', label: 'Date', description: 'Match the show date' },
    { id: 'time', label: 'Start Time', description: 'Review + finalize timing' },
  ];
  ...
  const canMoveForward = useMemo(() => {
    ...
    if (activeFlowStep === 1) {
      return artistSelection !== 'other' || Boolean(selectedArtistName.trim());
    }
    if (activeFlowStep === 2) {
      return Boolean(showLookupDate || effectiveShow || suggestedShow);
    }
    return Boolean(effectiveShow);
  }, [activeFlowStep, artistSelection, effectiveShow, selectedArtistName, showLookupDate, suggestedShow]);
  const goToFlowStep = (index) => setActiveFlowStep(Math.max(0, Math.min(index, flowSteps.length - 1)));
  const advanceFlow = () => {
    if (!canMoveForward) {
      return;
    }
    setActiveFlowStep((current) => Math.min(current + 1, flowSteps.length - 1));
  };
  const retreatFlow = () => setActiveFlowStep((current) => Math.max(current - 1, 0));
        ...
        <div className="flex-1">
          <div className="grid gap-3 md:grid-cols-4">
              {flowSteps.map((step, index) => {
                ...
                const isComplete = index < activeFlowStep || Boolean(index === 0 && photoMetadata);
                return (
                  <button
                    key={step.id}
                    type="button"
                    onClick={() => goToFlowStep(index)}
                  ...
                  >
                    <div className="text-[10px] font-semibold uppercase tracking-[0.2em] opacity-80">{step.label}</div>
                    <div className="mt-1 text-sm font-medium">{step.description}</div>
                  </button>
                );
              })}
          </div>
        ...
        <div className="space-y-4">
          {effectiveShow ? (
              ...
              <div className="rounded-2xl border border-dashed border-slate-800 bg-slate-950/60 p-6 text-center text-sm text-slate-400">
                Pick a show date and match to continue.
              </div>
          )}
        </div>
      ) : null}

      <div className="flex items-center justify-between gap-3 rounded-2xl border border-slate-800 bg-slate-950/70 p-3">
        <button
          type="button"
          onClick={retreatFlow}
